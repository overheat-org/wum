...
class CommandAnalyzer {
	...
	private analyzeEnum(path: NodePath<T.TSEnumDeclaration>) {
		throw new ExtnError('Cannot use enum in command', path);
	}

	private analyzeClass(path: NodePath<T.ClassDeclaration>) {
		throw new ExtnError('Cannot use class in command', path);
	}
	...
	private analyzeExportDefault(path: NodePath<T.ExportDefaultDeclaration>) {
		...
		throw new ExtnError(`Cannot export by default a non-command element`, path);
	}
	...
	private analyzeExportNamed(path: NodePath<T.ExportNamedDeclaration>) {
		...
		if(decl.isDeclaration()) {
			throw new ExtnError('Cannot export in command', decl);
		}
		...
		for(const specifier of path.get('specifiers')) {
			...
			throw new ExtnError('Cannot export in command', specifier);
		}
	}
}
